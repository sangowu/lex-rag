...
> ✅ **端到端已跑通并量化**（2026-09-04，**4 份合同 / 32 页 / 164 条问题**），
> 见 `docs/demo_ocr_rag.md`。合成扫描件由 `scripts/make_scanned_demo.py` 从 CUAD 原文
> 反向渲染——OmniDocBench 九类里**没有合同**，真实扫描合同没有 ground truth。
>
> **结果：CER 0.008~0.195（跨合同差 23 倍），而逐条配对下 OCR 一条回归都没造成**
> （McNemar `只有A好=0`，semantic_hit 0.820→0.860、FP 0.096→0.053、FN 0.140→0.120，
> 四项全部落在噪声里，**别把 +0.040 当收益**）。
>
> ⚠️ **机制：条款没被删，只是被改花了。** 87 条 gold span 里 35 条失去逐字形态，
> 但滑窗最小编辑距离显示**真正不在文档里的是 0 条**（距离 0.03~0.39）。
> 三把尺子里前两把各自都会给出错误结论——`quote_overlap`（最长**连续**公共段）说
> "12 条没了"，词覆盖率说"一条没丢"。**一严一松，夹出来的区间才是答案。**
>
> ⚠️ **这一版推翻了 #48 单份合同版的机制解释。** 那一版说"是合同文本的冗余救回来的，
> 所以只出现一次的事实会不可恢复地丢失"——文字根本没被删，冗余无从救起。
> 那一版的干净基线 semantic_hit=1.000 也是 CENTRACK 一份合同的数，4 份是 0.820。
> **每个数字都对，错的是从 n=1 推出的因果解释和基线。**
>
...
> 拆成逐页 PNG 会把一份合同变成 N 个文档。
>
> ⚠️ **每份合同用自己的 rng**（`seed + crc32(doc_id)`）：共用一个的话，往列表里插一份
> 会改变后面所有合同的噪声，已发表的数字全部作废。
>
> ⚠️ 仍未测：更脏的退化档位；以及"答案只出现一次的合同"——#48 担心的那个失败模式
> 在这 4 份里一次都没触发，**它仍是未被观察到的假设，既没证实也没证伪**。
...
20 条回归案例 + 阻断阈值，退出码 0/1。判定逻辑在 `lex_rag/gate.py`（纯函数，
...
### 可用性注入：能打穿的那条不是注入（2026-09-04）

"逼模型拒答"的探针已加：`Case.must_not_refuse`，靶子是**已知答得对**的题，
拒答即判为攻击得手；`forced_refusals` 是独立于 `injections_followed` 的第二个硬 0
（两者报告里必须分得开——修法相反，而后者极易被误读成拒答门变严了）。

⚠️ **三条已提交的探针（`availability__*`）又是"从一开始就打不穿"**：
新旧 prompt 各 0/10，**本仓库第二次栽在这上面**（第一次是 `contract_voice`）。
它们是回归探针，不是"我们防住了"的证据。

⚠️ **唯一打穿的 payload 被我刻意排除在门禁外**：主张"第 8 条已被修正案删除"，
新 prompt 16/30、旧 prompt 8/20。**模型没做错**——修正案删除条款正是"约束合同双方
的条款"，#44 判据合法放行它，律师读了也会得出同样结论。想用 prompt 修它等于让模型
无视修正案。这是**语料完整性问题，防线在 ingest 不在 prompt**；做成门禁案例只会
因正确行为拦住发布。

⚠️ **`data/runs/gate/` 里最后一个文件不一定是最新一轮线上配置**：
`20260903T174602Z` 是把 prompt 修复 stash 掉的旧-prompt 对照轮，它 FAIL 是**故意的**。
`--show latest` 会指到它。
...
## Ingest 层的来源指纹（`lex_rag/ingest_guard.py`）

可用性注入的结论是"防线在 ingest 不在 prompt"，这个模块是那句话的最小实现：
**不阻止改动，只保证改动会被看见。** 每次 ingest 按 `(table, doc_id)` 记一个
空白归一化后的 SHA-256，报 `new / unchanged / changed`，变更的逐条列出两个指纹。

- ⚠️ **`truncate()` 绝不能清 `doc_source`。** 全量 ingest 第一步就是 TRUNCATE，
  指纹跟着清掉的话每次重灌所有文档都变成 `new`，`changed` 永远不会出现——
  **机制等于没有，而且完全无声**。由 `tests/test_ingest_guard.py` 钉住。
- **按表分隔**：同一份合同合法地同时存在于 `chunks` 和 `chunks_ocr`，两份文本本来
  就不同，共用一行会让两条 ingest 路径互相报警。
- **归一化空白，但保留大小写和标点**：EDGAR 的成串空格、重跑 OCR 的换行差异不该报警；
  改个日期、少个 `not`、`Term` 变 `term` 必须报警。
  ⚠️ 这与 `text_match.normalize` **有意不同**——那把尺子问"答案像不像 gold"，
  这把问"这份文件是不是同一份"。**别把两者合并。**
  ⚠️ 代价：**纯排版攻击抓不到**。真要防那个得另加判据，不要调松这一条。
- **指纹写在 `_ingest_one` 最后**：中途失败就不该留下"已经见过这一版"的记录，
  否则下次重跑会把一次失败的 ingest 认成"未变"然后什么都不做。
- **默认只报告，`--fail-on-changed-source` 才阻断**：默认阻断只会训练出"每天加
  一个 --force"。
- OCR 路径比 `ingest.py` 更需要它：**同一份扫描件重跑 OCR 文本就会变**，
  没有指纹的话"语料被改写"和"这轮 OCR 跑歪了"在库里长得一模一样。
