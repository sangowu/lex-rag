...
- **reranker 重试是指数退避 + 抖动**（`_backoff_sec`），不是固定间隔。原来固定 1.0s 时，
  ...
  **实测原因已确认**：一轮全量触发 93 次重试，**全部是 `HTTP 429 "TPM limit reached"`**
  （每分钟 token 配额，不是单请求上限）。重试逐级吸收 62→21→7→3，93 次限流只剩
  1 条查询最终失败（修复前每轮 13~14 条）。要降 429 就得降 token 消耗：
  `rerank_top_k=60` 每轮都把整个候选池发出去，是第一去处。
...
**2 倍平均延迟换 1.4 个点召回**，28% 的查询付 10~17 秒。值不值得是产品取舍，
但现在这个取舍是在真实数字上做的。
